    ...
    class EMGCallbacks : IEMGCallbacks
    {
        public void NewPacket(XDataPacket packet)
        {
            ...
            for (Int32 i = 0; i < nFrames; i++)
            {
                ...
                sample[0]=frame[03]<<24|frame[04]<<16|frame[05]<<8; sample[0]=(sample[0]/256)*inputDifferentialVoltage/(float)(powerOfResBits*packet.gain);
                sample[1]=frame[06]<<24|frame[07]<<16|frame[08]<<8; sample[1]=(sample[1]/256)*inputDifferentialVoltage/(float)(powerOfResBits*packet.gain);
                sample[2]=frame[09]<<24|frame[10]<<16|frame[11]<<8; sample[2]=(sample[2]/256)*inputDifferentialVoltage/(float)(powerOfResBits*packet.gain);
                sample[3]=frame[12]<<24|frame[13]<<16|frame[14]<<8; sample[3]=(sample[3]/256)*inputDifferentialVoltage/(float)(powerOfResBits*packet.gain);
                sample[4]=frame[15]<<24|frame[16]<<16|frame[17]<<8; sample[4]=(sample[4]/256)*inputDifferentialVoltage/(float)(powerOfResBits*packet.gain);
                sample[5]=frame[18]<<24|frame[19]<<16|frame[20]<<8; sample[5]=(sample[5]/256)*inputDifferentialVoltage/(float)(powerOfResBits*packet.gain);
                sample[6]=frame[21]<<24|frame[22]<<16|frame[23]<<8; sample[6]=(sample[6]/256)*inputDifferentialVoltage/(float)(powerOfResBits*packet.gain);
                sample[7]=frame[24]<<24|frame[25]<<16|frame[26]<<8; sample[7]=(sample[7]/256)*inputDifferentialVoltage/(float)(powerOfResBits*packet.gain);

                currentFrame = new Frame(sample, countvalue, countvalue / (float) packet.sampleFreq,packet.inputDifferentialVoltageNegative,packet.inputDifferentialVoltagePositive);
