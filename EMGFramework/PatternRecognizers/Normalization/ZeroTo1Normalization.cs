...
{
    class ZeroTo1Normalization : Normalization
    {
        ...
        public override void RunOnline(double[] featureVector)
        {
            ...
            {
                featureVector[j] = (featureVector[j] + _shift[j]) / _delta[j];
                if (featureVector[j] < 0) featureVector[j] = 0;
                else if (featureVector[j] > 1) featureVector[j] = 1;
            }
        }
        ...
        public override void RunOffline(List<double[,]> featureVectors)
        {
            ...
            foreach (double[,] set in featureVectors)
            {
                ...
                for (int i = 0; i < dimi; i++)
                    for (int j = 0; j < dimj; j++)
                        set[i, j] = (set[i, j] + _shift[j]) / _delta[j];
